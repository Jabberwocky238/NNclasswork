...
class MABN1d(nn.Module):
    """Applied MABN over a 2D/3D input as described in the paper
    `Towards Stabilizing Batch Statistics in Backward Propagation of Batch Normalization`

    Args:
        channels: :math:`C` from an expected input of size :math:`(N, C, L)`
        B: the real batch size per GPU.
        real_B: The batch size you want to simulate. It must be divisible by B.
        自己写的，理解可能有问题
    """
    def __init__(self, channels, B=128, real_B=2048, eps=1e-5, momentum=0.98, warmup_iters=100):
        super(MABN1d, self).__init__()
        assert real_B % B == 0
        self.buffer_size = real_B // B
        self.register_parameter('weight', nn.Parameter(torch.ones(channels)))
        self.register_parameter('bias', nn.Parameter(torch.zeros(channels)))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.training:
            ...
            x = BatchNormFunction.apply(x, self.weight, self.bias, self.running_var, self.eps, 
                                        self.momentum, self.buffer_x2, self.buffer_gz, self.iters, 
                                        self.buffer_size, self.warmup_iters)
            x = rearrange(x.squeeze(3), 'b l c -> b c l')
            return x
        else:
            N, C, H, W = x.size()
            var = self.running_var.view(1, C, 1, 1)
...
if __name__ == '__main__':
    img = torch.randn((128, 64, 512))
    sss = MABN1d(64, 128, 1024)
    # sss = MABN2d(64, 128, 1024)
    out = sss(img)
    print(out.shape)
